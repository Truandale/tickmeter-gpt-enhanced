...
### ✅ Применение расширенных настроек в рантайме
- BPF фильтр теперь уважается переключателем: `ADVANCED:bpf_filter_enabled` — SetFilter вызывается только когда включено (GUI.cs, PacketStats.cs).
- Дедупликация пакетов в мульти-режиме теперь управляется `SETTINGS:dedup_multi_nic` (GUI.cs).
- Лимит строк Live View задействован: `ADVANCED:live_max_rows_enabled/live_max_rows` — список ограничивается и подрезается сверху (PacketStats.cs).
- Ограничение FPS оверлея: `ADVANCED:overlay_fps_enabled/overlay_fps` — таймер `ticksLoop.Interval` пересчитывается при старте и сразу после закрытия Advanced Settings (GUI.cs, SettingsForm.cs).


### 📈 График и overlay
- Производительность графика: оптимизирована отрисовка с корректными интервалами обновления
- Синхронизация настроек: исправлен таймер overlay для правильной синхронизации с настройками ping_interval
- Точность отображения: обеспечена согласованность значений пинга и графика
